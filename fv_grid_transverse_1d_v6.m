 function fout = fv_grid_transverse_1d_v6(parameters)
 
%One dimensional (z and 1 node laterally) grid for flowline ice sheet calculations. 
...
extra.psi.layer_height = [0.5; 0.25; 0.125; 0.125];

extra.psi.n_v = ratio_ver*[4; 4; 4; 8];%2*[10; 10; 10; 15];%[4; 4; 4; 6];%[10; 20];%

dz = extra.psi.layer_height./extra.psi.n_v;
dy = ratio_hor*dz(1)*ones(extra.psi.n_layers,1);                             %THIS CHANGES DEPENDING ON THE NUMBER OF LAYERS
...
extra.N.n_v(end) = extra.N.n_v(end) + 1;                                   % add bed as a node to the lowermost layer
Ndy = ratio_hor*Ndz(1)*ones(extra.psi.n_layers,1);                          %THIS CHANGES DEPENDING ON THE NUMBER OF LAYERS
extra.N.bd_y = extra.psi.bd_y;
extra.N.n_y = extra.psi.n_y;       
...
Tbdy = ratio_hor*Ndz(1)*ones(extra.Tb.n_layers,1); %[1.5*Tdz(1:2); 1.5*Tdz(1); 2*(1.5*Tdz(1))];% %this is the size of cells %THIS CHANGES DEPENDING ON THE NUMBER OF LAYERS
...
fout.n_nodes.tot_virtual = sum(fout.n_nodes.hor.*fout.n_nodes.vert) + 0*sum(2*fout.n_nodes.hor(2:end));
...
for ss = 1:grid.n_layers
   ...
   if ss == 1
    ...
    elseif ss>1
        index_la_virtual = sum(grid.n_y(1:ss-1).*grid.n_v(1:ss-1)) + 0*sum(4*grid.n_y(1:ss-1));
   end
...
%     coor_intermediate(:,1) = coor_intermediate(:,2)- dy(ss);
    ...
    if ss>1
        ...
        down_node_interface = reshape(repmat((index_la-grid.n_y(ss-1)+1:index_la),[index_split 1]),[ ],1);
        down_node.ver = [down_node_interface;down_node.ver];
        
        down_node_virtual.ver = down_node.ver; 
        ...
        up_node_virtual.ver = up_node.ver; 
        
        upup_node_virtual.ver = [sparse(1,1); upup_node_virtual.ver]; 
        ...
        %one node downstream
        index_node_hdown = [index_la-2*grid.n_y(ss-1)+1; index_node_hdown];
        coor_node_h_y_down_interface = reshape(repmat((fout.coor_nodes.y(index_la-grid.n_y(ss-1)+2:index_la)).',[index_split 1]),[ ],1);
        coor_node_h_y.down = [fout.coor_nodes.y(index_la-grid.n_y(ss-1)+1);  coor_node_h_y.down];
        
        %two nodes downstream
        index_node_hdowndown = [index_la-2*grid.n_y(ss-1)+1; index_node_hdowndown];
        coor_node_h_y.downdown = [fout.coor_nodes.y(index_la-grid.n_y(ss-1)+1); coor_node_h_y.downdown];
        
        %one node upstream
        index_node_hup = [index_la-2*grid.n_y(ss-1)+1; index_node_hup];
        coor_node_h_y.up = [fout.coor_nodes.y(index_la-grid.n_y(ss-1)+1);  coor_node_h_y.up];
        
        %two nodes upstream

        index_node_hupup = [index_la-2*grid.n_y(ss-1)+1; index_node_hupup];
        coor_node_h_y.upup = [fout.coor_nodes.y(index_la-grid.n_y(ss-1)+1);  coor_node_h_y.upup ];
        
        y_virtual_interface = fout.coor_nodes.y(index_la-grid.n_y(ss-1)+1);
        coor_node_virtual.y = [y_virtual_interface;  coor_node_virtual.y];
        coor_node_virtual.z = [[]; coor_node_virtual.z]; 
...
if flag==1
    fout.n_nodes.tot_virtual = sum(fout.n_nodes.hor.*fout.n_nodes.vert) + 0*sum(2*fout.n_nodes.hor(2:end));
elseif flag==0
    fout.n_nodes.tot_virtual = sum(fout.n_nodes.hor.*fout.n_nodes.vert) + 0*sum(2*fout.n_nodes.hor(1:end-1));
end
...
if flag == 1
    fout.bdy_nodes.bed = fout.n_nodes.vert;
end
fout.bdy_nodes.inflow = [];                                                %build along with the grid
fout.bdy_nodes.outflow = [];                                               %build along with the grid
...
fout.Delta_y_edge = [];
fout.Delta_z_cell = [];
fout.Delta_z_cell_volume = [];  %so, the difference between Delta_z_cell_volume and Delta_z_cell is that the former is the right cell vertical height for T(ice) and all the flow variables,
...
for ss = 1:grid.n_layers
    ...
    elseif ss>1
        if dz(2)<dz(1)
            index_la_virtual = sum(grid.n_y(1:ss-1).*grid.n_v(1:ss-1)) + 0*sum(2*grid.n_y(2:ss));
        else
            index_la_virtual = sum(grid.n_y(1:ss-1).*grid.n_v(1:ss-1)) + 0*sum(2*grid.n_y(1:ss-1));
        end
    end
    n_nodes_layer = grid.n_y(ss)*grid.n_v(ss);
    n_edges_layer_ver = grid.n_y(ss)*(grid.n_v(ss)-1);
    ...
    if ss >1
        ...
        n_edges_layer_a = n_nodes_layer; %this does include the interface?
        
        %horizontal advection averaging, network for u_vert (with
        %horizontal network edges)
        index_ve_nodes = grid.n_y(1).*(grid.n_v(1) -1) + grid.n_y(1) + sum(grid.n_y(2:ss-1).*(grid.n_v(2:ss-1)-1) + grid.n_y(2:ss-1)) ;%includes lower interface
        index_ve_edges = grid.n_y(1).*(grid.n_v(1) -1) + grid.n_y(1) + sum(grid.n_y(2:ss-1).*grid.n_v(2:ss-1)+ grid.n_y(2:ss-1));%includes lower interface
    ...
    % length of horizontal cell edges for computation of vertical fluxes (only interior vertical edges
    %n.b.: the row of nodes that lies on a layer boundary belongs to the lowest layer)
    ...
    %virtual node network
    index_node_hdown = index_nodes;
    coor_node_h_y.down = coor_nodes.y;
    
    %one further downstream
    index_node_hdowndown = index_nodes;
    coor_node_h_y.downdown = coor_nodes.y;
    
    %one upstream
    index_node_hup =index_nodes;
    coor_node_h_y.up =coor_nodes.y;
    
    %two upstream
    index_node_hupup = index_nodes;
    coor_node_h_y.upup = coor_nodes.y;
       
    
    %add ver network edges that connect lower to upper layer
    if ss>1 && flag == 1 %refinement
        ...
        down_node_virtual.ver = [down_node_interface; down_node_virtual.ver];
        down_node_adv_ver_av = [down_node_interface; down_node_adv_ver_av];

        down_node_adv_hor = [index_la-grid.n_y(ss-1)+1; down_node_adv_hor];
        up_node_adv_hor = [index_la-grid.n_y(ss-1)+1; up_node_adv_hor];
        ...
        up_node.ver = [index_la+1; up_node.ver];
        up_node_adv_ver_av = [index_ve_a + 1; up_node_adv_ver_av];
        
        %virtual network:
        up_node_virtual.ver = [index_la+1; up_node_virtual.ver];  
        
        %add location (x,z) of hor cell edges
        
        coor_hor_cell_edges.y = [dy(ss-1); coor_hor_cell_edges.y];
        coor_hor_cell_edges.z = [(z_top+dz(ss-1)/2)*ones(length(up_node_interface),1);coor_hor_cell_edges.z];
        
        %add surface crossed by vertical network edge
        delta_y_vflux_interface = dy(ss);
        ...
        %down node
        index_node_hddown_interface = reshape(repmat((index_la-grid.n_y(ss-1)+1:index_la),[index_split 1]),[ ],1);
        index_node_hudown_interface = index_la+1;
        index_node_hdown = [index_node_hddown_interface; index_node_hudown_interface; index_node_hdown];
        
        coor_node_h_y_d_down_interface = reshape(repmat((fout.coor_nodes.y(index_la-grid.n_y(ss-1)+1:index_la)).',[index_split 1]),[ ],1);
        coor_node_h_y_d_up_interface = coor_node_h_y_d_down_interface;
        coor_node_h_y.down = [coor_node_h_y_d_down_interface; coor_node_h_y_d_up_interface; coor_node_h_y.down];
        
        %upnode
        index_node_hdup_interface = index_la;
        index_node_huup_interface = index_la+1;
        index_node_hup = [index_node_hdup_interface; index_node_huup_interface; index_node_hup];
        
        coor_node_h_y_u_down_interface = dy(ss-1);
        coor_node_h_y_u_up_interface = dy(ss);
        ...
        index_node_hupup_interface = index_node_hup_interface;
        ...
        coor_node_hupup_interface = coor_node_hup_interface;
        ...
        index_node_hdowndown_interface = index_node_hdown_interface;
    ...
    elseif ss>1 && flag == 0 %coarsening as we move down along the grid
        %up nodes
        up_node_interface = reshape(repmat((index_la+1:index_la+grid.n_y(ss)),[index_split 1]),[ ],1);  %OK
        ...
        %down node 
        index_node_hudown_interface = up_node_interface;
        index_node_hddown_interface = down_node_interface;
        %index_node_hdown = [index_node_hddown_interface; index_node_hudown_interface; index_node_hdown];
        
        %coor_node_h_y.down = [dy(ss); dy(ss); coor_node_h_y.down];
        
         %upnode
        index_node_hup = [index_node_hddown_interface; index_node_hudown_interface; index_node_hup];
        %coor_node_h_y.up = [dy(ss); dy(ss); coor_node_h_y.up];
        
         %one node further up
        %coor_node_h_y.upup = [dy(ss); dy(ss); coor_node_h_y.upup];
        
        %one node further down
        index_node_hdowndown = [index_node_hddown_interface; index_node_hudown_interface; index_node_hdowndown];
        %coor_node_h_y.downdown = [dy(ss); dy(ss); coor_node_h_y.downdown];
